...
- **格式面板**:选中节点(Ctrl 可多选批量)改加粗、斜体、字号、字体、文字色、填充色、边框色与边框宽,一键恢复默认样式
- 文件:打开/保存/另存为 `.smm`(与官方格式互通)、拖拽文件到窗口打开
- **导入**:Markdown / XMind / OPML(多个顶层条目自动补一个虚拟根,不静默丢内容)
- **导出**:PNG / JPG / SVG / PDF / Markdown / XMind / OPML(PDF 固定 A4 并自动等比缩放,横向导图自动转横向)
- **自动保存**:改动防抖 800ms 直接落盘(先写临时文件再改名覆盖,进程被杀也不会留下半截文件),关窗前再同步补写一次;全程没有任何"是否保存"弹窗。新建/未命名文档只在首次改动时弹一次"另存为"让你选位置
- **不丢改动**:切换文档/新建/导出/关窗前,会先把"正在编辑的节点文字"和"备注输入框里还没提交的内容"收进文档再落盘;文件被其他程序占用而写不进去时会自动重试,仍失败就浮出提示条并**拦住这次切换**(可选择重试 / 另存为副本 / 明确丢弃),不再静默丢失
- **快照兜底**:每次成功保存都会往 `%APPDATA%\mind-map-desktop\snapshots\` 留一份(每个文件最多 20 份),菜单「文件 ▸ 打开快照文件夹」可直达,误覆盖时能回滚
- 左侧"最近打开"侧边栏(双击打开,可隐藏)+ 启动自动恢复上次编辑的导图
- 右侧节点备注侧边栏:Ctrl+T 或点击节点右上角泡泡编辑当前节点备注
- 节点右键菜单标记"已完成",对勾画在节点文本后方,随文件保存、可撤销、导出图片可见
- 主题切换:默认(浅)/深色/经典蓝/清新绿,打开文件时按文件内主题还原
- 视图:适应画布 / 放大 / 缩小;打开任意导图自动居中并在超出画布时缩小
- 原生菜单 + 快捷键:Ctrl+N / O / S / Shift+S / E(导出 PNG) / Z / Y / T / 0 / +/−
...
- 自动快照在 `%APPDATA%\mind-map-desktop\snapshots\<路径哈希>\`,每个文件最多留最近 20 份;这个目录只增不删(超出 20 份才清理最旧的),不放心的话可以定期自己清。
- 兼容性说明:"已完成"对勾写的是自定义节点字段 `data.data.done`,官方 Web 版不识别这个字段(数据不会丢,回到本客户端仍生效)。

## 已知限制

- 导入是"结构导入":md / xmind / opml 只带出文本、备注、超链接、标签,原文件的主题与样式不保留,导入后套用本客户端默认主题。
- 图片 / PDF / SVG 导出是整张画布的快照,导图很大时仍收在单页 A4 里(自动缩小,不拼接多页)。
- 未注册富文本(RichText)、公式(Formula)、小地图(MiniMap)等核心库插件。
- 自动保存无冲突检测:同一文件被两个窗口同时打开时,后落盘的一方会覆盖另一方。
- 上次打开的文件若被删除或移走,启动时会弹一次"文件解析失败"提示。
- 未做代码签名与应用图标。
...
- **Qoder**([@QoderAI](https://github.com/QoderAI))— AI 编程助手,参与方案设计、代码实现、调试与文档编写。

本项目的代码与打包产物为 AI 辅助完成。若发现缺陷请提 Issue,维护者会复核。
